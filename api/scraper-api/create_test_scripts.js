...
providers.forEach(p => {
  const providerPath = path.join(apiDir, p);
  if (!fs.existsSync(providerPath)) {
    return;
  }
  
  const testCode = `const BASE = 'http://localhost:9090';

async function log(msg) {
    console.log('[${p.toUpperCase()} Test] ' + msg);
}

async function testProvider() {
    try {
      log('1. Fetching latest movies/search...');
      let res = await fetch(\`\${BASE}/api/${p}\`).catch(() => null);
      if (!res || !res.ok) {
          log('Failed to fetch from /api/${p}. Status: ' + (res?.status || 'Unknown'));
          return;
      }
      ...
      if (!itemUrl && !firstItem.id) {
          log('Could not find URL or ID: ' + JSON.stringify(firstItem).substring(0, 50));
          return;
      }
      ...
      let detailsRes = await fetch(\`\${BASE}/api/${p}/details\${detailQuery}\`).catch(() => null);
      ...
      let extPath = \`\${BASE}/api/${p}/extractor\`;
      ...
      if (extRes && extRes.ok) {
         ...
         try {
           ...
           log('SUCCESS! Extracted: ' + JSON.stringify(extData).substring(0,200));
...
`;
  ...
  fs.writeFileSync(path.join(providerPath, 'test_extraction.js'), testCode);
});
