...
async function log(msg) {
    console.log('[NETMIRROR Test] ' + msg);
}

async function testProvider() {
    try {
      log('1. Fetching latest movies/search...');
      let res = await fetch(`${BASE}/api/netmirror?page=1`).catch(() => null);
      if (!res || !res.ok) {
          log('Failed to fetch from /api/netmirror. Status: ' + (res?.status || 'Unknown'));
          return;
      }
      ...
      let detailsRes = await fetch(`${BASE}/api/netmirror/details${detailQuery}`).catch(() => null);
      ...
      let extPath = `${BASE}/api/netmirror/extractor`;
