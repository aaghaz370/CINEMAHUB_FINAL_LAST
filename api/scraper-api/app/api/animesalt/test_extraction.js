...
async function log(msg) {
    console.log('[ANIMESALT Test] ' + msg);
}

async function testProvider() {
    try {
      log('1. Fetching latest movies/search...');
      let res = await fetch(`${BASE}/api/animesalt?page=1`).catch(() => null);
      if (!res || !res.ok) {
          log('Failed to fetch from /api/animesalt. Status: ' + (res?.status || 'Unknown'));
          return;
      }
      ...
      let detailsRes = await fetch(`${BASE}/api/animesalt/details${detailQuery}`).catch(() => null);
      ...
      let extPath = `${BASE}/api/animesalt/extractor`;
