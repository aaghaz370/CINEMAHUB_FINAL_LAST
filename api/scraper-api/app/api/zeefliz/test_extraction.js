...
async function log(msg) {
    console.log('[ZEEFLIZ Test] ' + msg);
}

async function testProvider() {
    try {
      log('1. Fetching latest movies/search...');
      let res = await fetch(`${BASE}/api/zeefliz?page=1`).catch(() => null);
      if (!res || !res.ok) {
          log('Failed to fetch from /api/zeefliz. Status: ' + (res?.status || 'Unknown'));
          return;
      }
      ...
      let detailsRes = await fetch(`${BASE}/api/zeefliz/details${detailQuery}`).catch(() => null);
      ...
      let extPath = `${BASE}/api/zeefliz/extractor`;
