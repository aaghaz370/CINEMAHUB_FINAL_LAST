...
async function log(msg) {
    console.log('[HDHUB4U Test] ' + msg);
}

async function testProvider() {
    try {
      log('1. Fetching latest movies/search...');
      let res = await fetch(`${BASE}/api/hdhub4u?page=1`).catch(() => null);
      if (!res || !res.ok) {
          log('Failed to fetch from /api/hdhub4u. Status: ' + (res?.status || 'Unknown'));
          return;
      }
      ...
      let detailsRes = await fetch(`${BASE}/api/hdhub4u/details${detailQuery}`).catch(() => null);
      ...
      let extPath = `${BASE}/api/hdhub4u/extractor`;
