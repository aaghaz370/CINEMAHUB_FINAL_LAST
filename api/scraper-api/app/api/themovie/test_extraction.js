...
async function log(msg) {
    console.log('[THEMOVIE Test] ' + msg);
}

async function testProvider() {
    try {
      log('1. Fetching latest movies/search...');
      let res = await fetch(`${BASE}/api/themovie?page=1`).catch(() => null);
      if (!res || !res.ok) {
          log('Failed to fetch from /api/themovie. Status: ' + (res?.status || 'Unknown'));
          return;
      }
      ...
      let detailsRes = await fetch(`${BASE}/api/themovie/details${detailQuery}`).catch(() => null);
      ...
      let extPath = `${BASE}/api/themovie/extractor`;
