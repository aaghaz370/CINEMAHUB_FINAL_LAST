...
async function log(msg) {
    console.log('[4KHDHUB Test] ' + msg);
}

async function testProvider() {
    try {
      log('1. Fetching latest movies/search...');
      let res = await fetch(`${BASE}/api/4khdhub?page=1`).catch(() => null);
      if (!res || !res.ok) {
          log('Failed to fetch from /api/4khdhub. Status: ' + (res?.status || 'Unknown'));
          return;
      }
      ...
      let detailsRes = await fetch(`${BASE}/api/4khdhub/details${detailQuery}`).catch(() => null);
      ...
      let extPath = `${BASE}/api/4khdhub/extractor`;
