...
async function log(msg) {
    console.log('[PH Test] ' + msg);
}

async function testProvider() {
    try {
      log('1. Fetching latest movies/search...');
      let res = await fetch(`${BASE}/api/ph`).catch(() => null);
      if (!res || !res.ok) {
          log('Failed to fetch from /api/ph. Status: ' + (res?.status || 'Unknown'));
          return;
      }
      ...
      let detailsRes = await fetch(`${BASE}/api/ph/details${detailQuery}`).catch(() => null);
      ...
      let extPath = `${BASE}/api/ph/extractor`;
