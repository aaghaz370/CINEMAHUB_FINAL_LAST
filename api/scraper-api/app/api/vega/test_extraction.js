...
async function log(msg) {
    console.log('[VEGA Test] ' + msg);
}

async function testProvider() {
    try {
      log('1. Fetching latest movies/search...');
      let res = await fetch(`${BASE}/api/vega?page=1`).catch(() => null);
      if (!res || !res.ok) {
          log('Failed to fetch from /api/vega. Status: ' + (res?.status || 'Unknown'));
          return;
      }
      ...
      let detailsRes = await fetch(`${BASE}/api/vega/details${detailQuery}`).catch(() => null);
      ...
      let extPath = `${BASE}/api/vega/extractor`;
