...
async function log(msg) {
    console.log('[CASTEL Test] ' + msg);
}

async function testProvider() {
    try {
      log('1. Fetching latest movies/search...');
      let res = await fetch(`${BASE}/api/castel?page=1`).catch(() => null);
      if (!res || !res.ok) {
          log('Failed to fetch from /api/castel. Status: ' + (res?.status || 'Unknown'));
          return;
      }
      ...
      let detailsRes = await fetch(`${BASE}/api/castel/details${detailQuery}`).catch(() => null);
      ...
      let extPath = `${BASE}/api/castel/extractor`;
