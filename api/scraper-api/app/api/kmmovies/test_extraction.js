...
async function log(msg) {
    console.log('[KMMOVIES Test] ' + msg);
}

async function testProvider() {
    try {
      log('1. Fetching latest movies/search...');
      let res = await fetch(`${BASE}/api/kmmovies?page=1`).catch(() => null);
      if (!res || !res.ok) {
          log('Failed to fetch from /api/kmmovies. Status: ' + (res?.status || 'Unknown'));
          return;
      }
      ...
      let detailsRes = await fetch(`${BASE}/api/kmmovies/details${detailQuery}`).catch(() => null);
      ...
      let extPath = `${BASE}/api/kmmovies/extractor`;
