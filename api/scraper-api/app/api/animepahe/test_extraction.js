...
async function log(msg) {
    console.log('[ANIMEPAHE Test] ' + msg);
}

async function testProvider() {
    try {
      log('1. Fetching latest movies/search...');
      let res = await fetch(`${BASE}/api/animepahe?page=1`).catch(() => null);
      if (!res || !res.ok) {
          log('Failed to fetch from /api/animepahe. Status: ' + (res?.status || 'Unknown'));
          return;
      }
      ...
      let detailsRes = await fetch(`${BASE}/api/animepahe/details${detailQuery}`).catch(() => null);
      ...
      let extPath = `${BASE}/api/animepahe/extractor`;
