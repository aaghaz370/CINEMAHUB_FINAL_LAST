...
async function log(msg) {
    console.log('[ADULT Test] ' + msg);
}

async function testProvider() {
    try {
      log('1. Fetching latest movies/search...');
      let res = await fetch(`${BASE}/api/adult`).catch(() => null);
      if (!res || !res.ok) {
          log('Failed to fetch from /api/adult. Status: ' + (res?.status || 'Unknown'));
          return;
      }
      ...
      let detailsRes = await fetch(`${BASE}/api/adult/details${detailQuery}`).catch(() => null);
      ...
      let extPath = `${BASE}/api/adult/extractor`;
