...
async function log(msg) {
    console.log('[HENTAI Test] ' + msg);
}

async function testProvider() {
    try {
      log('1. Fetching latest movies/search...');
      let res = await fetch(`${BASE}/api/hentai`).catch(() => null);
      if (!res || !res.ok) {
          log('Failed to fetch from /api/hentai. Status: ' + (res?.status || 'Unknown'));
          return;
      }
      ...
      let detailsRes = await fetch(`${BASE}/api/hentai/details${detailQuery}`).catch(() => null);
      ...
      let extPath = `${BASE}/api/hentai/extractor`;
