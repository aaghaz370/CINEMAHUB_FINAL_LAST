...
async function log(msg) {
    console.log('[UHDMOVIES Test] ' + msg);
}

async function testProvider() {
    try {
      log('1. Fetching latest movies/search...');
      let res = await fetch(`${BASE}/api/uhdmovies?page=1`).catch(() => null);
      if (!res || !res.ok) {
          log('Failed to fetch from /api/uhdmovies. Status: ' + (res?.status || 'Unknown'));
          return;
      }
      ...
      let detailsRes = await fetch(`${BASE}/api/uhdmovies/details${detailQuery}`).catch(() => null);
      ...
      let extPath = `${BASE}/api/uhdmovies/extractor`;
