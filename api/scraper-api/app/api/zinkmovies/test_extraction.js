...
async function log(msg) {
    console.log('[ZINKMOVIES Test] ' + msg);
}

async function testProvider() {
    try {
      log('1. Fetching latest movies/search...');
      let res = await fetch(`${BASE}/api/zinkmovies?page=1`).catch(() => null);
      if (!res || !res.ok) {
          log('Failed to fetch from /api/zinkmovies. Status: ' + (res?.status || 'Unknown'));
          return;
      }
      ...
      let detailsRes = await fetch(`${BASE}/api/zinkmovies/details${detailQuery}`).catch(() => null);
      ...
      let extPath = `${BASE}/api/zinkmovies/extractor`;
