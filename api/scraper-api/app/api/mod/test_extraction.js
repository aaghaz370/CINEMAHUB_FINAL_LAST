...
async function log(msg) {
    console.log('[MOD Test] ' + msg);
}

async function testProvider() {
    try {
      log('1. Fetching latest movies/search...');
      let res = await fetch(`${BASE}/api/mod?page=1`).catch(() => null);
      if (!res || !res.ok) {
          log('Failed to fetch from /api/mod. Status: ' + (res?.status || 'Unknown'));
          return;
      }
      ...
      let detailsRes = await fetch(`${BASE}/api/mod/details${detailQuery}`).catch(() => null);
      ...
      let extPath = `${BASE}/api/mod/extractor`;
