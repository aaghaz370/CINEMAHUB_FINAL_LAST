...
async function log(msg) {
    console.log('[DESIREMOVIES Test] ' + msg);
}

async function testProvider() {
    try {
      log('1. Fetching latest movies/search...');
      let res = await fetch(`${BASE}/api/desiremovies?page=1`).catch(() => null);
      if (!res || !res.ok) {
          log('Failed to fetch from /api/desiremovies. Status: ' + (res?.status || 'Unknown'));
          return;
      }
      ...
      let detailsRes = await fetch(`${BASE}/api/desiremovies/details${detailQuery}`).catch(() => null);
      ...
      let extPath = `${BASE}/api/desiremovies/extractor`;
